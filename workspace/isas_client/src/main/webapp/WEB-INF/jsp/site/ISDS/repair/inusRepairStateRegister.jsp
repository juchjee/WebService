...
// 			    		 var itmGubun = '${itmGubun}';
			    		 var item = $("#itmList").jqxComboBox('getSelectedItem')
			    		 var itmGubun = item.value;
...
// 			    		 var itmGubun = '${itmGubun}';
			...
			$("input[class*=number]").bind("keyup", function(){
			 ...
			 $("#as1Bc div.jqTransformSelectWrapper  a").click(function(){
				...
				if($("[name=as1Bc]").val() != ""){
			      fnAjax("ascodeList.action", {"asNo":'${param.asNo}',"subCd":$("[name=as1Bc]").val(),"asBc":'100'}, //20190507 asBc 추가 [A/S접수고장증상] 
...
// 				      fnAjax("getPartItemList.action", {"subCd":$("[name=partBc]").val()},
				      fnAjax("ascodeList.action", {"asNo":'${param.asNo}',"subCd":$("[name=partBc]").val(),"asBc":'300'},	//20190507 asBc 추가 [부위코드]
								function(data, dataType){
			   ...
			   //품목구분 선택 시, 품목구분별 모델 리스트 불러오도록 조정
			   $("#itmList").on('change', function (event){
					 var item = $("#itmList").jqxComboBox('getSelectedItem');
					 
					 if(item && item.value){
						 fnAjax("modelList.action", {"itmGubun":item.value},
							function(data, dataType){
			    	 			
							 $("#modelList").jqxComboBox({ selectedIndex: -1, source: data, displayMember: "label", valueMember: "value", width: 300, height: 36,	promptText: "모델을 선택하세요!"});
							
							 var item = $("#modelList").jqxComboBox('getItemByValue', '');
					         $("#modelList").jqxComboBox('selectItem', '');
							},"POST"
						);
					 }
	 			 }); 
			   ...
			   //품목구분 선택 시, 모델 초기화
// 			   $("#itmList").on('click', function (event){
// 				   $("#modelList").jqxComboBox('selectItem', '');
// 			   });
			 
			   
			 $("#modelList").on('keyup', function (event){
// 				 var scrollPosition = $("#modelList").offset().top;
...
// 				 },500);
			 ...
			 $("#okBtn").bind("click",function(){
					...
					var itm = $("#itmList").jqxComboBox('getSelectedItem');
					...
					//20190507 필수값체크 - 모델, A/S증상(대), A/S증상(중), 부위코드(대), 부위코드(중)
					var arr = [];
					arr = ["asTy","as1Bc","as2Bc","partBc","partBc2"];
					
					for(var i = 0; i < arr.length; i++) {
						if($("select[name=" + arr[i] + "]").val() == "") {
							alert($("select[name=" + arr[i] + "]")[0].title + "을(를) 선택해주세요!");
			 				return;
						}
					}
		...
		function fnDataSetting(){
            ...
            $("#itmList").jqxComboBox({ selectedIndex: -1, source: dataAdapter, displayMember: "label", valueMember: "value", width: 300, height: 36,	promptText: "모델을 선택하세요!"});
            var item = $("#itmList").jqxComboBox('getItemByValue', '${itmGubun}');
            $("#itmList").jqxComboBox('selectItem', '${itmGubun}');
	        
            var url = "modelList.action?itmGubun=${itmGubun}";
            // prepare the data
            var source =
            {
                datatype: "json",
            ...
            //20190509 itm_id가 아니라 ori_itm_id로 지정
            //기존 품목코드가 등록내역 모델에 셋팅되게끔 oriItmId 추가 
//          //var item = $("#modelList").jqxComboBox('getItemByValue', '${param.itmId}');
//          //$("#modelList").jqxComboBox('selectItem', '${param.itmId}');
            var item = $("#modelList").jqxComboBox('getItemByValue', '${param.oriItmId}');
            $("#modelList").jqxComboBox('selectItem', '${param.oriItmId}');
...
<body>
	...
	<input type="hidden" id="itmGubun" name="itmGubun" value=""/>
...
<!-- 					</dd> -->
<!-- 					<dt>부위코드(대)</dt> -->
					<dt>수리코드(대)</dt> <!-- 20190530 부위코드 > 수리코드로 명칭 변경 -->
					<dd id="partBc">
						<div class="select_guide">
<!-- 							<select class="m_select select01 validation[required]" name="partBc" id="ct_select_03" title="부위코드(대)"> -->
							<select class="m_select select01 validation[required]" name="partBc" id="ct_select_03" title="수리코드(대)"> <!-- 20190524 부위코드 > 수리코드로 명칭 변경 -->
								<option value="">선택하세요!</option>
...
<!-- 					<dt>부위코드(중)</dt> -->
					<dt>수리코드(중)</dt>	<!-- 20190530 부위코드 > 수리코드로 명칭 변경 -->
					<dd id="partBc2">
						<div class="select_guide">
<!-- 							<select class="m_select select01 validation[required]" name="partBc2" id="ct_select_04" title="부위코드(중)"> -->
							<select class="m_select select01 validation[required]" name="partBc2" id="ct_select_04" title="수리코드(중)"> <!-- 20190524 부위코드 > 수리코드로 명칭 변경 -->
								<option value="">선택하세요!</option>
							</select>
						</div>
					</dd>
					...
					<dd>
						<div class="input_txt_bx">
							<input type="text" name="rmks" id="rmks" class="validation[required]" title="대리점의견" /> <!-- 2019-08-09 추가 -->
						</div>
					</dd>
					<dt>비용구분(유/무)</dt>
					<dd id="amtTy">
