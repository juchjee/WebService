...
public class FileUpLoad implements IConstants{
			...
			for (int i = 0; i < uploadName.length; i++) {
				...
				for (int j = 0; j < value.length; j++){
					if(value[j] != null && value[j].IsUploaded()){
						...
						String fileName = RandomStringUtils.randomAlphanumeric(10) + System.currentTimeMillis() + "." + StringUtils.lowerCase(StringUtils.substringAfterLast(originalFileName, "."));
	...
	private UnCamelMap<String, Object> getFileMap(String attchFileNm, String domName, String savedFilePath, int imageSize){
		...
		String attchFilePath = attchAbsolutePath.replace(imageUploadPath, imagesUploadUrl);
