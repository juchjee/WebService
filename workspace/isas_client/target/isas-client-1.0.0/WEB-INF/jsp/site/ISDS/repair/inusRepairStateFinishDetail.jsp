	...
	<!--
		...
		function fnEvent(){
			$("#smsSend").bind("click",function(){
				if(confirm("${repairStateView.tHp}로 문자를 발송하시겠습니까?")){
		 			fnAjax("smsSend.action", {"mbrNm":"${repairStateView.tNm}","mbrHp":"${repairStateView.tHp}","sum":$("#tot").html(),"reqDt":'${repairStateView.reqDt}'},
		...
		function fnDataSetting(){
			...
			if("${repairStateView.amtTyCd}" == "AS215100"){
				$("#custAmt").html(0);
				$("#custAmt2").html(setComma("${repairStateView.custAmt}"));
				...
				sum = sum + cAmt;
				$("#custAmt").html(setComma("${repairStateView.custAmt == null ? 0 : repairStateView.custAm}"));
				$("#custAmt2").html(0);
				$("#tot").html(setComma(sum));
			}
...
<body>
	<section class="sub cont">
		<div class="tit_bx">
			<a href="javascript:" onclick="location.href='/ISDS/repair/inusRepairStateFinish.do';" class="btn_prev">이전 화면</a>
			<h2>A/S 완료현황 상세</h2>
		</div>
		<!--// tit_bx -->
					...
					<table>
						...
						<c:forEach items="${repairPartList}" var="repairPart" varStatus="loop">
							<tr>
								...
								<td class="code">${repairPart.itmCd}</td>
								<td class="name">${repairPart.itmNm}</td>
		...
		<div class="btnBox pd15">
			<a href="javascript:;" id="smsSend" class="btn">영수 문자 보내기</a>
		</div>
	</section>
	<!--// sub -->
</body>