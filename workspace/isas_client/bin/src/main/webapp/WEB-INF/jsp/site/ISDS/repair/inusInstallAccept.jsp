...
<head>
	...
	function fnEvent(){
		$.fnDetail = function(insNo){
			makeForm("inusInstallAcceptDetail.do", {"page": "${param.page}", "insNo": insNo});
		}
...
<body>
	<section class="sub cont">
		...
		<div class="sch_opt_bx">
			<form name="frmSearch" method="post" action="/ISDS/repair/inusInstallAccept.do">
				<input type="hidden" name="dateFlag" id="dateFlag" value="" >
				<input type="hidden" id="page" name="page" value="${page}"/>
			<table>
				<caption>설치접수내역 검색</caption>
				<tbody>
					<tr>
					...
					<tr>
						...
						<td>
							<div class="select_guide">
								<!-- <select class="m_select select01" name="#" id="ing"> -->
								<select class="m_select select01" id="insStateCd" name="insStateCd">
									<option value="AS410100" <c:if test="${insStateCd eq 'AS204100'}">selected</c:if>>접수</option>
									<option value="AS410200" <c:if test="${insStateCd eq 'AS410200'}">selected</c:if>>대기</option>
									<option value="AS410300" <c:if test="${insStateCd eq 'AS410300'}">selected</c:if>>완료</option>
									<option value="AS410400" <c:if test="${insStateCd eq 'AS410400'}">selected</c:if>>취소</option>
									<option value="AS410500" <c:if test="${insStateCd eq 'AS410500'}">selected</c:if>>정산</option>
									<option value="AS410600" <c:if test="${insStateCd eq 'AS410600'}">selected</c:if>>취소(환경적)</option>
								</select><!-- m_select -->
							</div><!-- select_guide -->
						</td>
					</tr>
		...
		<!--// sch_opt_bx -->
		<c:forEach items="${installAcceptList}" var="installAccept" varStatus="loop">
			<div class="ing_bx">
				<a href="javascript:" onclick="$.fnDetail('${installAccept.insNo}');">
					<div class="box">
						<div class="date">${installAccept.insDt}</div>
						<dl class="situ">
							<dt>제품</dt>
							<dd>${installAccept.model}</dd>
							<dt>성명</dt>
							<dd>${installAccept.csNm}</dd>
							<dt>전화번호</dt>
							<dd>${installAccept.hp}</dd>
							<dt>진행상태</dt>
							<dd class="fc-red fc-bold">${installAccept.insStateNm}</dd>
						</dl>
					</div>
				</a>
			</div>
		</c:forEach>
		<c:if test="${empty installAcceptList}">
