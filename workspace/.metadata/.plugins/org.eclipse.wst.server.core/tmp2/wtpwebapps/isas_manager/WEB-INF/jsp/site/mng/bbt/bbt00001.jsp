...
<script type="text/javaScript" defer="defer">
	var contUrl = "${rootUri}${conUrl}bbt00001";
	...
	var _columns = [
		...
		,{ text: '분류', datafield: 'boardCateNm',  width: '10%', cellsalign: 'center', align: 'center'<c:if test="${fn:length(cateList) == 1}">,hidden: true</c:if>}
		,{ text: '제목', datafield: 'boardTitle',  <c:choose><c:when test="${fn:length(cateList) > 1}">width: '40%'</c:when><c:otherwise>width: '50%'</c:otherwise></c:choose>, cellsalign: 'left', align: 'center'}
		,{ text: '등록일', datafield: 'regDt',  width: '15%', cellsalign: 'center', align: 'center'}
		...
		,{ text: '수정', datafield: 'popButton', width: '5%', cellsalign: 'center', align: 'center',
			columntype: 'button', cellsrenderer: function () {
                ...
                var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', row);
					$.fancybox.open({
						href: "bbt00001R.action?boardMstCd=${boardMstCd}&boardSeq="+ dataRecord.boardSeq,
						type: "iframe",
						maxWidth	: 1920,
						maxHeight	: 1100,
						width		: 1000,
	...
	function fnEvent(){
		...
		$("#registBtn").click(function(){
			$("#registBtn").attr("href","bbt00001R.action?boardMstCd=${boardMstCd}");
		});
		...
		$("#jqxgrid").on('celldoubleclick', function (event)
				{
					...
					$("#boardSeq").val(datarow.boardSeq);
						setTimeout(function(){
							$("#modifyBtn").attr("href","bbt00001R.action?boardMstCd=${boardMstCd}&boardSeq="+datarow.boardSeq);
							$("#modifyBtn").click();
						},200);
					}
				});
				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<th scope="row">우선공지정렬</th>
							<td>
								<input type="checkbox" name="boardFirstYn" id="boardFirstYn" />
							</td>
						</tr>
					</tbody>
				</table>
