...
<head>
	...
	<!--
		...
		function fnExecEvent() {
			...
			var eCopnProdMpgArr = data["eCopnProdMpgArr"];
			if(eCopnProdMpgArr){
				if(eCopnProdMpgArr.length > 0){
					for (var i = 0; i < eCopnProdMpgArr.length; i++) {
						var obj = eCopnProdMpgArr[i];
						if(obj){
							$("#workForm").append(makeInput("eCopnProdMpgArr", obj['prodCd']));
							$("#eCopnProdMpg").append("<li><div>"+obj['prodNm']+"</div></li>");
						}
					}
				}
			}
		...
		function save() {
			...
			var eCopnProdMpgArr = $("input:hidden[name='eCopnProdMpgArr']");//상품명
			if (eCopnProdMpgArr.length < 1) {
				alert(message(MESSAGES_ERR.LENGTH_FAIL, [ '상품', '하나' ]));
				$("#eCopnProdA").focus();
				return false;
			}
...
<body class="noBg">
	...
	<form id="workForm" name="workForm" action="amM402PSave.action" method="post" >
		<input name="copnCd" type="hidden" value="${copnCd}">
		<input name="copnTpPdmc" type="hidden" value="P">
		<div class="table_type2">
			<table>
				...
				<tbody>
					...
					<tr>
						<th scope="row">
							<a id="eCopnProdA" class="fancybox bis btn_type1" data-fancybox-type="iframe" href="${rootUri}${mngUri}popup/prodSearchPop.do?tp=eCopnProd">상품검색</a>
						</th>
						<td>
							<div class="fl add_field"  style="line-height:28px;">
				              <div class="field_ul">
				                <ul id="eCopnProdMpg"></ul>
				              </div>
				            </div>
						</td>
					</tr>
