...
function fn_back() {
	...
	var empNo = $("#empNo").val();
	var facCd = $("#facCd").val();
	var outDt = searchDate($('#outDt').val());
	...
	location.assign("/mobile/scm/orderDelivery/mSalesShipR.do" + "?deptCd=" + deptCd + "?empNo=" + empNo + "?facCd=" + facCd + "?outDt=" + outDt);	
}
...
function fn_search(){
	...
	mobileAjax("/mobile/scm/orderDelivery/mSalesShipDtlR/list",obj,function(data){
		var d = data[0];
		//출고일자, 출고사업장, 출고사원, 현장명, 납품장소, 품목코드, 품목명, 수량, 금액, 차량번호, 운전자, 핸드폰
		$("#outDtR").text(d.outDt);
		$("#facNm").text(d.facNm);
		$("#empNm").text(d.empNm);
		$("#fldNm").text(d.fldNm);
...
<input type="hidden" id="outDt" name="outDt" value="">
</form>
...
<table class="dhx_table">
	<tbody>
		...
		<tr>
			...
			<td class="dhx_right_lbl"><span id="outDtR"></span></td>
		</tr>
		<tr>
		...
		<tr>
			<td class="dhx_left_lbl">담당자</td>
			<td class="dhx_right_lbl"><span id="empNm"></span></td>
		</tr>
		<tr>
