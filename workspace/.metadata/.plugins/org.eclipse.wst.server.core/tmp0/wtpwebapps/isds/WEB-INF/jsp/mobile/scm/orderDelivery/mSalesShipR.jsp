...
function fn_EmpCd(){
	...
	var empNo = $$('empNo');
	var mask = $$('deptCd').data.text;
	
	
// 	emp.getList().filter(function(opt) {
// // 		return opt.text.match(new RegExp("^"+mask.toLowerCase(), "i"))!=null;
// 		var rt = opt.value.match(new RegExp("^"+$$('deptCd').data.text.toLowerCase(), "i"))!=null;
...
// 	},false, mask);
	...
	mobileAjax("/mobile/scm/orderDelivery/mSalesShipR/salesEmpCombo",obj,function(data){
// 		list.define('datatype', 'json');
// 		list.define('data', data);
// 		list.adjust();
// 		list.refresh();
...
// 					$$('empNo_list').add(value.id, value.value);
// 					$$('empNo_list').$view.innerHTML += html;
// 					debugger;
// 				});
// 			}
...
// // 			empNo.adjust();
// 			var htm = '<div class="dhx_scroll_cont" touch_scroll="y" style="transform: translate3d(0px, 0px, 0px);">';
// 			$$('empNo_list').$view.children[0].setAttribute('style', 'transform: translate3D(0px,0px,0px)');
// 			empNo.refresh();
// // 			empNo.hideOverlay();
