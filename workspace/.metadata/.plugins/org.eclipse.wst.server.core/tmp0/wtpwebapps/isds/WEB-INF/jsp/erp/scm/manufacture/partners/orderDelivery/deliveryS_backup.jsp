...
$(document).ready(function(){
    ...
    gridMain.attachEvent("onSelectStateChanged",onSelectStateChanged);
...
function doOnRowSubSelect(id,ind){
	var deliKey = gridSub.setCells(id,gridSub.getColIndexById("deliKey")).getValue();
	if(deliKey == ''){
		var miDeliQtyIdx = gridSub.getColIndexById("miDeliQty");
		 var deliQtyIdx = gridSub.getColIndexById("deliQty");
		
		for(var i=0;i<gridSub.getRowsNum();i++){
			var miDeliQty = gridSub.setCells2(i,miDeliQtyIdx).getValue()*1;
			var deliQty = gridSub.setCells2(i,deliQtyIdx).getValue()*1;
			if(deliQty > miDeliQty){
				dhtmlx.alert("납품수량이 미납수량보다 큽니다.");
				gridSub.setCells2(i,deliQtyIdx).setValue(miDeliQty);
				gridSub.selectRow(i);
				break;
			}
		}
...
function fn_mainSearchCB(data){
	if(data != ''){		
		for(var i=0;i<gridMain.getRowsNum();i++){
			...
			if(statBc == "MM110100"){
				gridMain.dxObj.cells(gridMain.getRowId(i),gridMain.getColIndexById("statBcNm")).setBgColor('#FFFFFF');  
			}else if(statBc == "MM110300"){
				gridMain.dxObj.cells(gridMain.getRowId(i),gridMain.getColIndexById("statBcNm")).setBgColor('#FFE400'); 
			}else if(statBc == "MM110500"){
				gridMain.dxObj.cells(gridMain.getRowId(i),gridMain.getColIndexById("statBcNm")).setBgColor('#ABF200'); 
			}else if(statBc == "MM110700"){
				gridMain.dxObj.cells(gridMain.getRowId(i),gridMain.getColIndexById("statBcNm")).setBgColor('#ff0000'); 
			}else if(statBc == "MM110900"){
				gridMain.dxObj.cells(gridMain.getRowId(i),gridMain.getColIndexById("statBcNm")).setBgColor('#FF5E00'); 
			}
		}
	}
};
...
function onSelectStateChanged(id){
	for(var i=0;i<gridMain.getRowsNum();i++){
		var statBc = gridMain.setCells2(i,gridMain.getColIndexById("statBc")).getValue();
		if(statBc == "MM110100"){
			gridMain.dxObj.cells(gridMain.getRowId(i),gridMain.getColIndexById("statBcNm")).setBgColor('#FFFFFF');  
