	...
	<div class="sub">
		...
		<div class="box_guide">
			<c:choose>
				...
				<c:when test="${boardMstCd eq 'BBM00059'}">
					<h2 class="tit">시공점 찾기</h2>	
					<p class="desc">이누스 전국 시공점을 소개합니다.</p>
				</c:when>
				<c:otherwise>
					<h2 class="tit"></h2>
				</c:otherwise>
			...
			<div class="page_location">
				<ul>
					...
					<c:choose>
						...
						<c:when test="${boardMstCd eq 'BBM00059'}">
							<li class="last"><a href="#"><span id="kkk"></span>시공점 찾기</a></li>
						</c:when>
						<c:otherwise>
		...
		<div class="tblType02">
			<div class="box_tit">
				<c:choose>
					<c:when test="${boardMstCd eq 'BBM00057'}">
						<p id="kjyxz"class="tit">전체 전시장 <span>${totalCnt}</span>개</p>
					</c:when>
					<c:when test="${boardMstCd eq 'BBM00058'}">
						<p id="kjyxz"class="tit">전체 A/S센터 <span>${totalCnt}</span>개</p>
					</c:when>
					<c:when test="${boardMstCd eq 'BBM00059'}">
						<p id="kjyxz"class="tit">전체 시공점 <span>${totalCnt}</span>개</p>
					</c:when>
					<c:otherwise>
						<p id="kjyxz"class="tit">전체 <span>${totalCnt}</span>개</p>
					</c:otherwise>
				</c:choose>
			</div>
			<div class="store_bx">
				<div class="list_bx">
					<c:forEach items="${mapList}" var="mapList" varStatus="loop">
						...
						<div class="lst">
							...
							<!-- <p>서울시 강남구 학동로 140<br>(논현동)해농빌딩 B1~2F</p>
							<p>평일 : 10시~8시</p> -->
