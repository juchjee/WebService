...
<head>
	...
	<script type="text/javascript" defer="defer">
		...
		function fnSearch(){
			...
			if($("#searchTxt2").val() != ""){
				$("#searchTxt2").val($.trim($("#searchTxt2").val()));
			}
			if($("#searchTxt3").val() != ""){
				$("#searchTxt3").val($.trim($("#searchTxt3").val()));
			}
			frmSearch.submit();
		}
...
<body>
	<section class="sub cont">
		...
		<div class="sch_opt_bx">
			<form name="frmSearch" method="post" action="/ISDS/repair/inusRepairState.do">
				<input type="hidden" name="dateFlag" id="dateFlag" value="" >
				<input type="hidden" id="page" name="page" value="${page}"/>
				<table>
					<caption>A/S 접수현황 검색</caption>
					<tbody>
						<tr>
...
<!-- 										<option value="">전체</option> -->
		...
		<!--// sch_opt_bx -->
		<c:forEach items="${repairStateList}" var="repairState" varStatus="loop">
			<div class="ing_bx">
				<a href="javascript:" onclick="$.fnDetail('${repairState.asNo}');">
					<div class="box">
						<div class="date">${repairState.recDt}</div>
						<dl class="situ">
							<dt>제품</dt>
							<dd>${repairState.model}<c:if test="${not empty repairState.modelItemCd}">(${repairState.modelItemCd})</c:if></dd>
							<dt>성명</dt>
							<dd>${repairState.tNm}</dd>
							<dt>전화번호</dt>
							<dd>${repairState.tHp}</dd>
							<dt>진행상태</dt>
							<dd class="fc-red fc-bold">${repairState.statBcNm}</dd>
						</dl>
					</div>
				</a>
			</div>
		</c:forEach>
		<c:if test="${empty repairStateList}">
