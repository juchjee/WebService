...
<head>
	...
	<title>CheckPlus 안심본인인증 테스트</title>
 ...
 */
				...
				<c:choose>
					<c:when test="${param_r1 eq 'id' or param_r1 eq 'pwd'}">
						window.opener.location.href = '/ISDS/mm/searchEnd.do?mbrDi=${mbrDi}';
					</c:when>
					<c:when test="${param_r1 eq 'join'}">
						window.opener.location.href = '/ISDS/mm/join.do?mbrDi=checkData';
					</c:when>
					<c:when test="${param_type eq 'cs' }">
						window.opener.location.href = '/ISDS/cs/${param_r2}.do';
					</c:when>
					<c:when test="${param_type eq 'csAjax'}">
						//window.opener.location.href = '/ISDS/mm/join.do?mbrDi=checkData';
					</c:when>
					<c:when test="${param_type eq 'root' }">
						window.opener.location.href = '/';
					</c:when>
					<c:otherwise>
						window.opener.location.href = '/ISDS/mm/acessLogin.do';
					</c:otherwise>
				</c:choose>
			}else{
			...
			<c:choose>
				<c:when test="${param_r1 eq 'id' or param_r1 eq 'pwd'}">
					parent.location.href = '/ISDS/mm/searchEnd.do?mbrDi=${mbrDi}';
				</c:when>
				<c:when test="${param_r1 eq 'join'}">
					parent.location.href = '/ISDS/mm/join.do?mbrDi=checkData';
				</c:when>
				<c:when test="${param_type eq 'cs' }">
					parent.location.href = '/ISDS/cs/${param_r2}.do';
				</c:when>
				<c:when test="${param_type eq 'csAjax'}">
					//window.opener.location.href = '/ISDS/mm/join.do?mbrDi=checkData';
				</c:when>
				<c:otherwise>parent.location.href = '/';</c:otherwise>
			</c:choose>
