...
// 			);
			...
			fnAjax("ascodeList.action", {"codeTp":codeTp,"itmGubun":$.lvalue,"lvalue":ctData,"asBc":"100"},	//20190507 asBc 추가 [A/S증상대분류]
				function(data, dataType){
						$("#ascodeLev1").html("<option value=''>선택하세요</option>");
		...
		/*제품 변경 후 로딩 시*/
		$("#ctSelectDiv2 li a").click(function(){
			adLev1=0;

			var value = $(this).parent().index();
			$.lvalue = $("[name=itmGubunList]").attr('selectedIndex', value).val();
			 $("[name=itmGubun]").val($.lvalue);
...
// 			);
			...
			fnAjax("ascodeList.action", {"codeTp":codeTp,"itmGubun":$("[name=itmGubunList]").attr('selectedIndex', value).val(),"lvalue":""},
				function(data, dataType){
						$("#ascodeLev1").html("<option value=''>선택하세요</option>");
						$("#ascodeSelectDiv span").html('선택하세요');
						$("#ascodeSelectDiv ul").html("<li><a href='javascript:;' index='0' class='selected'>선택하세요</a></li>");
...
// 			);
		...
		/*----------------------------- 전송이벤트 - 저장버튼 클릭 이벤트 Start -----------------------------*/
		 $("#okBtn").bind("click",function(){
			...
			var mbrNm = $("input[name='mbrNm']").val();
			$("#mbrNm").val(mbrNm);
...
<!-- 		<input type="hidden" name="csTimeSeq" id="csTimeSeq" class="validation[required]" title="예약시간"/> -->
		...
		<input type="hidden" name="csTimeSeq" id="csTimeSeq" title="예약시간"/>
			...
			<div class="box">
				...
				<div class="cont">
					<dl class="type01 hr">
						...
						<dt class="star" id="dtItm" style="display:none;">품목선택</dt>
						<dd id="ddItm" style="display:none;">
							<div class="select_guide" id="ctSelectDiv2">
								<select class="m_select select01"  name="itmGubunList" id="itmGubunList" title="품목">
										<option value=''>선택하세요!</option>
									<c:forEach items="${itmGubunList}" var="itmGubun" varStatus="status">
										<option value='${itmGubun.value}'>${itmGubun.label}</option>
									</c:forEach>
								</select>
						...
						<dd class="h140px ">
							<div class="txtArea pb10">
								<textarea id="txtArea" name="comment" onkeyUp="fnByteCheck(this,'1000');"></textarea>
								<p></p>
		...
		<div class="ing_bx nobg">
			<div class="box">
				...
				<div class="cont">
					...
					<dl class="type01 hr" id="nameArea">
						<dt class="star">성명</dt>
						<dd>
						...
						<dd>
							<div class="input_txt_bx">
								...
								<input type="text" name="mbrMobile" placeholder="-빼고 숫자만 입력" id="mbrMobile" value="${mbrMobile1}${mbrMobile2}${mbrMobile3}" class=""/>
								...
								<%--
 								<html:selectList name='mbrMobile1' id='mbrMobile1' <c:if test="${!empty mbrMobile1}">selectedValue="${mbrMobile1}"</c:if><c:if test="${empty mbrMobile1}">selectedValue="010"</c:if> optionValues='|010|011|016|017|018|019' optionNames='선택|010|011|016|017|018|019' script='class="pay_input_size07"'/>
								<input type="text" id="mbrMobile2" name="mbrMobile2" value="${mbrMobile2}" class="validation[numberOnlyCenter]" maxlength="4" style="position:relative;">
								<input type="text" id="mbrMobile3" name="mbrMobile3" value="${mbrMobile3}" class="validation[numberOnlyCenter]" maxlength="4" style="position:relative;">
								--%>
							</div>
						</dd>
						<dt class="star">주소</dt>
						...
						<dt>첨부파일</dt>
<!-- 					<dt class="star">첨부파일</dt> --> <!-- 20191119 주석처리 -->
						<dt>첨부파일</dt>
						<dd class="hauto">
		...
		<div class="ing_bx nobg">
			<div class="box">
<!-- 				<div class="tit drop">Step2. 고객 정보 입력<span class="star">필수입력사항</span></div> -->
<!-- 				<div class="tit drop">Step3. 예약 일정 입력<span class="star">필수입력사항</span></div> -->
				<div class="tit drop">Step3. 예약 일정 입력</div>
				<div class="cont">
					<dl class="type02">
						<dt>예약날짜 선택</dt>
						<dd>
							<div class="dateWrap">
								...
								<div class="month pdl10" id="calendar">
								</div>
							</div><!--// dateWrap -->
						</dd>
						<dt>예약시간 선택</dt>
					...
					<div class="choice_bx">
						<dl>
							<dt>선택일 :</dt>
							<dd><span id="selectText" class="fc-blue">선택하세요</span></dd>
						</dl>
						<dl>
							<dt>선택시간 :</dt>
							<dd><span id="selectTimeText" class="fc-blue">선택하세요</span></dd>
						</dl>
					</div>
					--%>
					<div class="box"></div> <!-- 20191119 추가 -->
					<p class="mark">
						고객 센터 운영시간은 평일 9시~18시입니다.<br>
		                (단, 토/일,공휴일은 휴무 , 점심시간 12시~13시 제외)
		                <%-- <br>
						선택하신 예약시간보다 전문상담사의 전화가 다소 늦어질 수 있습니다. --%> <!-- 20191119 주석처리 -->
		            </p>
				</div>
				<!--// cont -->
			</div>
		...
		<div class="btnWrap wide pd15">
			...
			<a href="javascript:;" id="okBtn" class="btn blue">예약신청</a>
		</div>
	</section>
	<!--// sub -->
	</form>
