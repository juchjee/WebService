...
public class CommonUtil implements IConstants{
	...
	@SuppressWarnings("rawtypes")
	public static String nvlArrVal(Object arg, int index){
		if(isEmpty(arg)){
			return "";
		}else{
			if(arg instanceof String[]){
				...
				return nvl(linkedList.get(index));
	...
	public static void setCookie(HttpServletResponse response, String name, String value, int age) {
		...
		Cookie cookie = new Cookie(name, StringEscapeUtils.escapeJava(aes256Value)); // 쿠키 생성
		...
		if (cookies != null) {
			for (int i = 0; i < cookies.length; i++) {
				if (cookies[i].getName().equals(name)) {
					String unescape = StringEscapeUtils.unescapeJava(cookies[i].getValue());
					String aes256Value = AES256Cipher.aesDecryptCbc(unescape);
	 ...
	 * @return
	 */
	public static String dateStrToNewDateStr(String dateStr, String nowPattern, String newPattern){
		String reDateStr = "";
		try {
			Date date = new Date();
			DateFormat dateFormat = new SimpleDateFormat(nowPattern, Locale.getDefault());
			date = dateFormat.parse(dateStr);
			SimpleDateFormat formatter = new SimpleDateFormat(newPattern, Locale.getDefault());
			reDateStr = formatter.format(date);
		} catch (ParseException e) {}
		return reDateStr;
	}
	/**
