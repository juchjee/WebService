...
public class EgovComCrossSiteHndlr extends BodyTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		//logger.info("EgovComCrossSiteFilter> ============================");
		try {
			// print value if available; otherwise, try 'default'
			if (value != null) {
				//logger.info("EgovComCrossSiteFilter> =value");
				String sWriteEscapedXml = getWriteEscapedXml();
				//logger.info("EgovComCrossSiteFilter sWriteEscapedXml>" + sWriteEscapedXml);
				...
				//logger.info("EgovComCrossSiteFilter def> ="+def);

				// if we do have 'default', print it
				if (def != null) {
					// good 'default'
					out(pageContext, escapeXml, def);
				//logger.info("EgovComCrossSiteFilter> ="+def);
				}
				return SKIP_BODY;
			}
	...
	public int doEndTag() throws JspException {
		try {
			//logger.info("EgovComCrossSiteFilter ==== doEndTag");
			if (!needBody){
			...
			if (bodyContent != null && bodyContent.getString() != null){
				...
				//logger.info("EgovComCrossSiteFilter> end");
				//logger.info("EgovComCrossSiteFilter sWriteEscapedXml > sWriteEscapedXml");
		...
		for(int i = 0; i < length; i++) {
			...
			if (c <= Util.HIGHEST_SPECIAL) {
				...
				if (escaped != null) {
					...
					// add escaped xml
					//sRtn = sRtn + escaped;
					//logger.info(buffer[i]+" :: " + escaped);
					for (int j = 0; j < escaped.length; j++) {
						//logger.info(buffer[i]+" :>: " + escaped[j]);
						sRtn = sRtn + escaped[j];
					}
    ...
    public static void main(String[] args) throws IOException
    {
    	...
    	//logger.info("writeEscapedXml " + egovComCrossSiteHndlr.getWriteEscapedXml());
    	/*
    	logger.info("sCrossSiteHndlr|"+ sCrossSiteHndlr + "|");

    	try{
    		logger.info("TRY TEST 1");
    		throw new Exception();
    	}catch(Exception e){
    		logger.info("TRY TEST 2");
    	}finally{
    		logger.info("TRY TEST 3");
