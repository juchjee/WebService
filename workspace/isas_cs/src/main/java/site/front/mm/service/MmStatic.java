...
public class MmStatic implements IConstants{
	 ...
	 * @return the reurl
	 */
	public static String getJoinUrl(int index, HttpServletRequest request) {
		
		if(CommonUtil.isMobile(request)){
			if(index == 1){ //가입정보 입력
				return "join_m";
			}else if(index == 2){ //가입완료
				return "joinEnd_m";
			}else if(index == 3){ //이용약관동의
				return "joinTerms_m";
			}else{
				return "joinClause_m"; //본인인증 // ★서버 올릴때 : 주석풀것, 퍼블리싱에서 휴대폰 본인인증 화면이 없어 회원가입 작성 화면으로 임시 이동 
				//return "join_m";//★서버 올릴때 : 개발 후 제거 요망
			}
		}else{
			if(index == 1){
				return "join";
