...
$(document).ready(function(){
    ...
    gridMain.addHeader({name:"RFID_NO",   colId:"rfidNo",    width:"150",   align:"left",    type:"ed"});
    ...
    gridMain.addHeader({name:"접수수량",    colId:"itemQn",     width:"60",   align:"right",  type:"edn"});
    ...
    gridMain.init();
    gridMain.cs_setColumnHidden(["sqNo","costId","scmYn","orderQn","deliSeq","jungsanYn","oldItemQn","rfidNoOld","rfidsqNoOld"]);
	...
	if(data != ''){
		for(var i=0;i<gridMain.getRowsNum();i++){
			...
			if(inputcDt != '' || outputcDt != ''){
				...
				gridMain.setCellExcellType(gridMain.getRowId(i),gridMain.getColIndexById("rfidNo"),"ro");
				gridMain.setCellExcellType(gridMain.getRowId(i),gridMain.getColIndexById("itemQn"),"ron");
			}
		}
	}
}
...
function gridMainAddRow(){
	...
	if(rfidNo == ''){
		...
		rfidFlag = false;
		$('#rfidNo').val('');
	}else if(rfidNoLen != 10){
			dhtmlx.alert({ok:"확인",text:"RFID번호가 10자리가 아닙니다. 확인해주세요.",
			     callback:function(){
			     $('#rfidNo').val('');
			     $('#rfidNo').focus();
			}});
	...
	if(rfidFlag){
		var totalColNum = gridMain.getColumnCount();
		 var cnt = 1;
	      ...
	      var oldItemQnColIdx = gridMain.getColIndexById('oldItemQn');
		...
		data[rfidsqNoColIdx] = '1';
	  	gridMain.addRow(data);
	  	
	  	$('#rfidNo').val('');
	  	
	  	window.setTimeout(function() {
	  		fn_save(); //20181128 요청으로 추가
	    }, 100)	//0.1초 [1초:1000]
	}
};
...
function fn_saveCB(data){
	...
	fn_remiconSumQty();
	var itemQnVal = $('#itemQn').val()*1;
	var checkQty = 0;
	var oldCheckQty = 0;
	for(var c = 0;c<gridMain.getRowsNum();c++){
		var cudKeyVal = gridMain.setCells2(c,gridMain.getColIndexById("cudKey")).getValue();
		if(cudKeyVal == 'INSERT' || cudKeyVal == 'UPDATE'){
			checkQty = checkQty + gridMain.setCells2(c,gridMain.getColIndexById("itemQn")).getValue()*1;
			oldCheckQty = oldCheckQty + gridMain.setCells2(c,gridMain.getColIndexById("oldItemQn")).getValue()*1;
		}
	}
	if(cnt != 0){
		...
		return true;
	}else if((sumItemQn-oldCheckQty)+checkQty > itemQnVal){
		var msgQty = (sumItemQn-oldCheckQty)+checkQty;
		dhtmlx.alert("접수수량("+msgQty+")이 청구수량("+itemQnVal+")을 초과할수 없습니다.");
		return true;
	}else{
		fn_saveRfidNo();
		...
		if(scmYn == 1){
		   ...
		   $.ajax({
		    	url:  "/erp/scm/work/partners/remicon/remiconS/gridMainSaveScm",
		        type: "POST",
		        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
		        data: params,
		        dataType: "json",
		        beforeSend: function() {
		            subLayout.cells("c").progressOn();
		        },
		        success: function(data,status, xhr, settings) {
		        	if(xhr.getResponseHeader("EXCEPTION")=="Y") {
		      			...
		      			//20191028 ryul 김동후 과장님 요청으로 저장 메시지 팝업 주석처리 - 현장요청
		      			//dhtmlx.alert("저장 되었습니다.");
			        	fn_search(); 	
		      		}    
		        },
		        complete: function() {
		        	subLayout.cells("c").progressOff();
		        }
		   });
		}else{
			var jsonStr = gridMain.getJsonUpdated();
		   ...
		   var params = $("#pForm").serialize();  
		   $.ajax({
		    	url:  "/erp/scm/work/partners/remicon/remiconS/gridMainSave",
		        type: "POST",
		        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
		        data: params,
		        dataType: "json",
...
function fn_saveRfidNo(){
	for(var i=0;i<gridMain.getRowsNum();i++){
		var cudKey = gridMain.setCells2(i,gridMain.getColIndexById("cudKey")).getValue();
		var chkRfid = gridMain.setCells2(i,gridMain.getColIndexById("rfidNo")).getValue();
		var chkRfidOld = gridMain.setCells2(i,gridMain.getColIndexById("rfidNoOld")).getValue();
		if((cudKey == 'INSERT' || cudKey == 'UPDATE') && (chkRfid != chkRfidOld)){
		if(i == 0){
			var rfidNo = gridMain.setCells2(i,gridMain.getColIndexById("rfidNo")).getValue()*1;
			fn_rfidSqNoChk(rfidNo);
			var rfidSqNo = rfidSqNoVal;
			gridMain.setCells2(i,gridMain.getColIndexById("rfidsqNo")).setValue(rfidSqNo);
			rfidSqNoVal = null;
		}else{
			var cnt = 1;
			var rfidNo = gridMain.setCells2(i,gridMain.getColIndexById("rfidNo")).getValue()*1;
			...
			var rfidSqNo = rfidSqNoVal;
			for(var j=0;j<i;j++){
				var subRfidNo = gridMain.setCells2(j,gridMain.getColIndexById("rfidNo")).getValue()*1;
				if(rfidNo == subRfidNo){
					cnt++;
				}
			}
			if(cnt == 1){
				...
				if(rfidSqNo > 1){
					for(var j=0;j<i;j++){
						var subRfidNo = gridMain.setCells2(j,gridMain.getColIndexById("rfidNo")).getValue()*1;
						var subRfidSqNo = gridMain.setCells2(j,gridMain.getColIndexById("rfidsqNo")).getValue();
						if(rfidNo == subRfidNo && rfidSqNo == subRfidSqNo){
							rfidNoSqFlag = false;
					...
					if(rfidNoSqFlag){
						...
						subRfidSqNo = subRfidSqNo+1;
						 gridMain.setCells2(i,gridMain.getColIndexById("rfidsqNo")).setValue(subRfidSqNo);
					}
				}else{
				   gridMain.setCells2(i,gridMain.getColIndexById("rfidsqNo")).setValue(cnt);
				}
			}
...
function fn_remiconSumQty(){
	...
	gfn_callAjaxComm(obj, "/erp/scm/work/partners/remicon/remiconS/selRemiconSumItemQn", fn_remiconQtyCB, '');
};

function fn_remiconQtyCB(data){
	if(data != ''){
		sumItemQn = data[0].itemQn;
	}else{
		sumItemQn = 0;
	}
};
...
function fn_delete(){
	...
	if(inputcDt != '' || outputcDt != ''){
		...
		var cudKey = gridMain.setCells(rodid,gridMain.getColIndexById("cudKey")).getValue();
		if(cudKey == 'INSERT'){
