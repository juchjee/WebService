...
//코드 맵핑
function fnCodeMpg(form,id,obj){
	//obj 없으면 삭제버튼이벤트만 활성
	if(obj != undefined){
	var overlapChk = true;
	 ...
	 $.each(obj, function (index) {
			...
			if(id != 'setMpg'){
				// 중복 키워드 확인
				$("input[name="+id+"Arr]").each(function(idx1, value){
					 if(tVal == this.value){
			 				overlapChk = false;
					 }
				});
		 ...
		 if(overlapChk == true){
			 $("#"+form).append(makeInput(id+"Arr", this.value));
	    	 $("#"+id).append("<li><div>"+this.label+"<a href=\"javascript:void(0);\"><img src=\"/images/btn_keyword_del.png\" alt=\"삭제\"></a></div></li>");
		 }
		 overlapChk = true;
      });
	}
	...
	$("#"+id+" a").bind("click",function() {
		...
		//현재 인덱스를 변수에 정의
		var idx = $("#"+id+" a").index(this);

		$("#"+form).find("input[name="+id+"Arr]").eq(idx).remove();
		$("#"+id+" > li").eq(idx).remove();
...
//리스트그리드 코드 맵핑
function fnGridCodeMpg(form,id,obj){
	//obj 없으면 삭제버튼이벤트만 활성
	if(obj != undefined){
	var overlapChk = true;
	 ...
	 $.each(obj, function (index) {
		 ...
		 if(overlapChk == true){
			 $("#"+form).append(makeInput(id+"Arr", tVal));
	    	 $("#"+id).append("<li><div>"+tTxt+"<a href=\"javascript:void(0);\"><img src=\"/images/btn_keyword_del.png\" alt=\"삭제\"></a></div></li>");
		 }
		 overlapChk = true;
    });
	}
...
function fnAjax(_url, _data, _successFn, _type, _dataType, _async, _errorFn, _timeout){
	...
	if($.type(_type) == 'string' && _type.length > 2){
		...
		if(_upperType == 'GET' || _upperType == 'POST'){
...
function fnFormValidation(form){
	var valid = true;
	...
	var validNm = "";
	var loopFlag = "on";
	$("form[name='"+form+"'] [class*=validation]").each(function(index, value){
		...
		if($(this).is(":disabled") == false){
				...
				if($("form[name='"+form+"'] [class*=validation]").attr("class").indexOf("groupChoose") > -1){
					...
					if($("form[name='"+form+"'] [class*='("+targetGroupName+")']").length == groupIdx){
	...
	if($("form[name='"+form+"'] [class*=validation]").length >  0){
...
function fnValidation(){
	...
	$("[class*=validation]").each(function(index, value){
		...
		if($(this).is(":disabled") == false){
				...
				if($(this).val() != ""){
					if($(this).attr("class").indexOf("minLength:(") > -1){
						var minLength = $(this).attr("class").split("minLength:(")[1].split(")")[0];
						if($(this).val().length < minLength){
							alert(validNm + $.word+"최소 "+minLength+"자리 이상 작성해야 합니다.");
							this.focus();
							return valid = false;
						}
					}
				...
				if($(this).val() != ""){
					if($(this).attr("class").indexOf("minNumber:(") > -1){
						var minNumber = $(this).attr("class").split("minNumber:(")[1].split(")")[0];
						if(parseInt(removeComma($(this).val())) < minNumber){
							alert(validNm + $.word+"최소 "+minNumber+"이상 등록할 수 없습니다.");
							this.focus();
							return valid = false;
						}
					}
				...
				if($(this).val() != ""){
					if($(this).attr("class").indexOf("maxNumber:(") > -1){
						var maxNumber = $(this).attr("class").split("maxNumber:(")[1].split(")")[0];
						if(parseInt(removeComma($(this).val())) > maxNumber){
							alert(validNm + $.word+"최대 "+maxNumber+"이상 등록할 수 없습니다.");
							this.focus();
							return valid = false;
						}
					}
				...
				//사업자번호 확인
				if($(this).val() != ""){
					if($(this).attr("class").indexOf("bizNo") > -1){
						if(!isValidSaupNo(removeComma($(this).val()))){
							this.focus();
							return  valid =  false;
						}
					}
				...
				//법인번호 확인
				if($(this).val() != ""){
					if($(this).attr("class").indexOf("sRegNo") > -1){
						if(!isValidsRegNo(removeComma($(this).val()))){
							this.focus();
							return  valid =  false;
						}
					}
...
function isValidsRegNo(sRegNo)
{
 sRegNo = sRegNo.replace('-','');

 if (sRegNo.length != 13){
	alert("법인번호는 13자리 숫자입니다.");
	return false;
 }
...
//사업자번호 자동완성
function setBizNo(val){
	var chkVal = removeComma(val);
	if(chkVal.length > 5)
	{
		return chkVal.substring(0,3)+"-"+chkVal.substring(3,5)+"-"+chkVal.substring(5,10);
	}

	if(chkVal.length > 3)
	{
		return chkVal.substring(0,3)+"-"+chkVal.substring(3,5);
	}else{
		return chkVal;
	}
}
//법인번호 자동 완성
function setSRegNo(val){
	var chkVal = removeComma(val);
	if(chkVal.length > 6)
	{
		return chkVal.substring(0,6)+"-"+chkVal.substring(6,13);
	}else{
		return chkVal;
	}
}
...
//일반 전화번호 정규식
var regExp5 = /^\d{2,3}-\d{3,4}-\d{4}$/;
//아이디나 비밀번호 정규식
var regExp6 = /^[a-z0-9_]{4,20}$/;
//휴대폰번호 체크 정규식
var regExp7 = /^01([0|1|6|7|8|9]?)-?([0-9]{3,4})-?([0-9]{4})$/;
 ...
 * @param obj
 */
function getTargetDate(targetDate, d, m, y, type) {
	var now = new Date();
	if(targetDate != null){
	...
	if(type != null){
		if(type == 'start'){
			str = new Date(year, month, day, '00', '00', '00')
		}else if(type == 'end'){
			str = new Date(year, month, day, '23', '59', '59')
		}else if(type == 'now'){
			str = new Date(year, month, day, now.getHours(), now.getMinutes(), now.getSeconds())
		}
	}else{
