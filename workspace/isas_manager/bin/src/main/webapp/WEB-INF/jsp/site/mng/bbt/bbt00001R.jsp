...
<script type="text/javaScript" defer="defer">
	...
	var oEditors = [];
	var contUrl = "${rootUri}${conUrl}bbt00001R";
	...
	$(window).load(function(){
		...
		<c:if test="${empty viewMap.boardSeq}">$("#boardCont").css("height","340px");</c:if>
...
<body class="noBg" style="height:788px;">
	<div class="popup_wrap">
		...
		<div class="pageContScroll_st4">
			...
			<form id="workForm" name="workForm" action="bbt00001Save.action" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="bbtCd" id="bbtCd" value="bbt00001" />
				<table>
					...
					<tbody>
						<tr>
							...
							<td>
								...
								<c:if test="${empty viewMap.boardSeq || viewMap.dataUserTpMa eq 'A'}">
							    (우선공지) : <input type="checkbox" name="boardFirstYn" id="boardFirstYn" value="Y" <c:if test="${viewMap.boardFirstYn eq 'Y'}">checked='checked'</c:if> />
							    </c:if>
							</td>
							<th scope="row">작성일</th>
							<td>
							...
							<td colspan="3">
								<div class="edit_area">
									<div class="editScroll" >
										<textarea name="boardCont" id="boardCont" class="validation[required]" title="내용" style="width:100%; height:340px;">${viewMap.boardCont}</textarea>
						            </div>
								</div>
							</td>
						</tr>
