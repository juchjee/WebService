...
function fnDataSetting(){
 		...
 		dateTimeInput("popupStartDt", option, "${popupInfo.popupStartDt}");
 	  	dateTimeInput("popupEndDt", option,"${popupInfo.popupEndDt}");
		...
		$("input[name=popupRegTpFe]:radio[value='${popupInfo.popupRegTpFe}']").attr("checked",true);
		  $("input[name=popupStatusYn]:radio[value='${popupInfo.popupStatusYn}']").attr("checked",true);
		  $("input[name=popupOpenTpWlm]:radio[value='${popupInfo.popupOpenTpWlm}']").attr("checked",true);

		  $("input[name=popupCenterYesNo]:checkbox[value='${popupInfo.popupCenterYesNo}']").attr("checked",true);

			$("input[name=popupOpenTpWlm]:radio[value='${popupInfo.popupOpenTpWlm}']").click();

		  $("#colorPicker").jqxColorPicker({ color: "${popupInfo.popupCloseColor}", colorMode: 'hue', width: 220, height: 220});
          $("#dropDownButton").jqxDropDownButton({ width: 85, height: 26});
          $("#dropDownButton").jqxDropDownButton('setContent', getTextElementByColor(new $.jqx.color({ hex: "${popupInfo.popupCloseColor}" })));
		$("input[name=popupRegTpFe]:radio[value='${popupInfo.popupRegTpFe}']").click();
}
</script>
</head>
...
<body class="noBg" style="height:580px">
	...
	<input type="hidden" name="popupSeq" id="popupSeq" value="${popupInfo.popupSeq}" />
		...
		<table>
			...
			<tbody>
				<tr>
					...
					<td>
						<input type="text" name="popupTitle" class="validation[required]" style="width:80%;" value="${popupInfo.popupTitle}" />
					</td>
				</tr>
				<tr>
				...
				<tr  class="uploadView" >
					...
					<td>
						<input type="text" name="popHtmlName" class="verT inputPx200" style="width:200px" readonly="readonly" value="${popupInfo.attchFileNm}" />
				...
				<tr class="editView"  >
					...
					<td>
						<div class="edit_area">
								<textarea name="popupCont" id="popupCont" class="validation[required]" title="에디터등록"  style="width:100%;height:300px;display:none;">${popupInfo.popupCont}</textarea>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class="fl">
							가로크기 : <input type="text" name="popupWinWidth" class="validation[required,number]" title="창(가로크기)" style="width:40px;" placeholder="0" value="${popupInfo.popupWinWidth}" /> pixel
						</div>
						<div class="fl marL30">
							세로크기 : <input type="text" name="popupWinHeight" class="validation[required,number]" title="창(세로크기)" style="width:40px;" placeholder="0" value="${popupInfo.popupWinHeight}" /> pixel
						</div>
					</td>
				</tr>
				<tr class="winPosition">
					...
					<td>
						<div class="fl">
							상단에서 : <input type="text" name="popupWinTop" class="validation[number]" title="창위치(상단에서)" style="width:40px;" placeholder="0" value="${popupInfo.popupWinTop}" /> pixel
						</div>
						<div class="fl marL30">
							좌측에서 : <input type="text" name="popupWinLeft" class="validation[number]" title="창위치(좌측에서)" style="width:40px;" placeholder="0" value="${popupInfo.popupWinLeft}" /> pixel
						</div>
				...
				<tr>
					...
					<td>
						<div class="fl" style="line-height:26px;">
							닫는 기간 <input type="text" name="popupCloseDay" id="popupCloseDay" class="marR5 validation[number]" placeholder="1"  style="width:40px;" value="${popupInfo.popupCloseDay}"  />일
						</div>
						...
						<div class="fl marL30" style="line-height:26px;">
							글씨 크기 : <input type="text" name="popupCloseSize" id="popupCloseSize" class="marR5 validation[number]"  placeholder="12"  style="width:40px;"  value="${popupInfo.popupCloseSize}" />px
						</div>
					</td>
				</tr>
				<tr>
		  ...
		  <div class="btn_area">
		    <div class="center">
		      ...
		      <a class="btn_blue_line2" href="javascript:;" onclick="$.submit();">
			      <c:if test="${not empty popupInfo.popupSeq}">수정</c:if>
			      <c:if test="${empty popupInfo.popupSeq}">등록</c:if>
		      </a>
		    </div>
		  </div>
	</div>
