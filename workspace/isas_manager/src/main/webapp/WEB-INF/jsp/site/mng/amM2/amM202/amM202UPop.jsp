	...
	$( "#prodCateReg" ).bind("click",function() {
		...
		if(overlapChk == true){
			// 3. 저장될 카테고리 맵핑 데이터 input 추가
				...
				$("#categoryMpg").append("<li><div>"+returnLabel+"<a href=\"javascript:;\"><img src=\"${rootUri}images/btn_keyword_del.png\" alt=\"카테고리 삭제\"></a></div></li>");
	 ...
	 $("#prodTypeGsp").bind("change",function() {
		...
		/*----------------------------- 판매기간 - 상시제품 여부에 따른 변경 이벤트 Start -----------------------------*/
		 ...
		 /*----------------------------- 상품종류변경시 추가정보,상세정보,고시사항 변경 이벤트 Start -----------------------------*/
			...
			/*----------------------------- 상세 이미지 삭제 이벤트 Start -----------------------------*/
			$.dtlImgDel = function(attchCd,idx){
				if(confirm("삭제하시겠습니까?")){
					fnAjax("dtlImgDel.action", {"attchCd" : attchCd}, function(data, dataType){
						$(".nowDtlImg").eq(idx).html("");
...
function fnDataSetting(){
	...
	<c:if test="${prod.prodPmntYn eq 'Y'}">
		$("input[name=prodPmntYn]").click(); //상시상품여부 YN
	</c:if>

	  // datepicker 정의
	  dateTimeInput("prodFormDt", null,"${prod.prodFormDt}");
	  $("input[name=prodFormDt]").addClass("validation[required]");
	  $("input[name=prodToDt]").addClass("validation[required]");
	  ...
	  <c:forEach items="${prodIconMpgList}" var="prodIconMpg">
	 	 $("input[name=prodIcon]:checkbox[value='${prodIconMpg.prodIconCd}']").attr("checked",true);
	  </c:forEach>

		//기본이미지 초기설정
	  <c:forEach items="${prodBasicImgMpgSize}" var="prodBasicImgMpgS">
		 $("input[name=prodBasicSizeBmsView]:checkbox[value='${prodBasicImgMpgS.prodBasicSizeBms}']").attr("checked",true); //현재 기본이미지분류(대중소)
	  ...
	  $("select[name=prodTypeGsp]").val("${prod.prodTypeGsp}").attr("selected", "selected"); //현재 상품구성
	  ...
	  <c:choose>
		  <c:when test="${prod.prodTypeGsp ne 'P'}">  //-----일반/셋트상품일때
			  ...
			  $("input[name=prodTaxTpTf]:radio[value='${prodFi.prodTaxTpTf}']").attr("checked",true);  //과세구분(T :  과세 , F : 면세)
...
//	 		  $("#prodToDt").val("${prodFi.prodToDt}"); //판매종료일
		  ...
		  $("select[name=prodDeliPlicyCd]").val("${prodPt.prodDeliPlicyCd}").attr("selected", "selected"); //현재 배송정책상태
		  </c:otherwise>
	  </c:choose>
	  ...
	  $("#prodProgressQty").text(setComma("${prodInventory.prodProgressQty}")+" 개"); //판매진행수
		...
		fnCodeMpg("workForm","setMpg",null);
		fnCodeMpg("workForm","secMpg",null);
		fnCodeMpg("workForm","keywordMpg",null);
		fnCodeMpg("workForm","prodMpg",null);
		  //--------------------------------------------------- 초기화 기본 데이터 End --------------------------------------------------
}
...
<body class="noBg">
	<div class="popup_wrap" style="background:#fff">
	 ...
	 <div class="pageContScroll_st2">
	 	...
	 	<input type="hidden" name="prodCd"  value="${prod.prodCd}" />
	 	<input type="hidden" name="transTp"  value="udpPop" />
		      ...
		      <div class="field_ul">
		        <ul id="categoryMpg">
		      		<c:forEach items="${prodCateMpgList}" var="prodCateMpg">
		        		<li>
		        			<div>${prodCateMpg.prodCategoryNm}<a  href="javascript:;"><img src="${rootUri}images/btn_keyword_del.png" alt="카테고리 삭제"></a></div>
		        			<input type="hidden" name="categoryMpgArr" value="${prodCateMpg.prodCategoryCd}"/>
		        		</li>
		        	</c:forEach>
		        </ul>
		      </div>
		    ...
		    <table>
		      ...
		      <tbody>
		          ...
		          <td colspan="3">
		            <div class="add_field">
		              <select id="prodTypeGsp" name="prodTypeGsp" style="width:154px;">
		                <c:choose>
			              	<c:when test="${prod.prodTypeGsp eq 'G'}">
				                <option value="G" >일반상품</option>
			                </c:when>
			              	<c:when test="${prod.prodTypeGsp eq 'S'}">
		              ...
		              <c:if test="${prodSetList == null}">
						<a id="prodSetBtn" class="fancybox bis btn_type1" data-fancybox-type="iframe" href="/mng/popup/prodSearchPop.do?tp=set">세트묶음선택</a>
					  </c:if>
		              <div class="field_ul">
		                <ul id="setMpg">
		                	<c:forEach items="${prodSetList}" var="prodSetMpg">
				        		<li>
				        			<div>${prodSetMpg.prodNm}<%-- <a  href="javascript:;"><img src="${rootUri}images/btn_keyword_del.png" alt="삭제"></a> --%></div>
				        			<input type="hidden" name="setMpgArr" value="${prodSetMpg.prodSetCd}"/>
				        		</li>
		        			</c:forEach>
		                </ul>
		              </div>
		        ...
		        <tr>
		          <th scope="row">상품명</th>
		          <td>
		            <input type="text" id="prodNm" name="prodNm"  style="width:600px" class="validation[required]"  value="${prod.prodNm}"  maxlength="50" title="상품명" />
		          </td>
		        </tr>
		        <tr>
		              ...
		              <div class="field_ul">
		                <ul id="secMpg">
		                	<c:forEach items="${secProdMpgList}" var="secMpg">
				        		<li>
				        			<div>${secMpg.secProdNm}<a  href="javascript:;"><img src="${rootUri}images/btn_keyword_del.png" alt="삭제"></a></div>
				        			<input type="hidden" name="secMpgArr" value="${secMpg.secProdCd}"/>
				        		</li>
		        			</c:forEach>
		                </ul>
		              </div>
		              ...
		              <div class="field_ul">
		                <ul id="keywordMpg">
		                	<c:forEach items="${keywordMpgList}" var="keywordMpg">
				        		<li>
				        			<div>${keywordMpg.prodKeywordNm}<a  href="javascript:;"><img src="${rootUri}images/btn_keyword_del.png" alt="삭제"></a></div>
				        			<input type="hidden" name="keywordMpgArr" value="${keywordMpg.prodKeywordCd}"/>
				        		</li>
		        			</c:forEach>
		                </ul>
		              </div>
		              ...
		              <div class="field_ul">
		                <ul id="prodMpg">
		                	<c:forEach items="${prodMpgList}" var="prodMpg">
				        		<li>
				        			<div>${prodMpg.prodNm}<a  href="javascript:;"><img src="${rootUri}images/btn_keyword_del.png" alt="삭제"></a></div>
				        			<input type="hidden" name="prodMpgArr" value="${prodMpg.assoProdCd}"/>
				        		</li>
		        			</c:forEach>
		                </ul>
		              </div>
		        ...
		        <tr>
		          ...
		          <td>
		            <input type="text" id="prodDesc" name="prodDesc" style="width:600px" value="${prod.prodDesc}"/>
		          </td>
		        </tr>
		      </tbody>
		    </table>
		      ...
		      <tbody  id="prodAddInfo">
		         <c:forEach items="${prodAddInfoList}" var="prodAddInfo">
		          <tr>
			          ...
			          <td>
			            <input type="hidden"  name="prodAddInfoCd" style="width:154px" value="${prodAddInfo.prodAddInfoCd}" />

						<c:choose>
							<%-- 문자타입 일때 --%>
							<c:when test="${prodAddInfo.prodAddTpSn eq 'S' }"><input type="text"  name="prodAddInfoVal" style="width:600px" value="${prodAddInfo.prodAddInfoVal}"  maxlength="250" /></c:when>
							<%-- 숫자타입 일때 --%>
							<c:when test="${prodAddInfo.prodAddTpSn eq 'N' }"><input type="text"  name="prodAddInfoVal" class="alignR validation[number]" style="width:50px" placeholder="0" value="${prodAddInfo.prodAddInfoVal}" /></c:when>
						</c:choose>
		        ...
		        <tr>
		          ...
		          <td colspan="3">
		          	<div id="prodBasicImgNameView">
		          		<div style="line-height:80px;">현재이미지 :
		          				  ...
		          				  <input type="checkbox" id="imgLNow" name="prodBasicSizeBmsView" class="marL30"   value="B" disabled="disabled"/><label for="imgLNow" class="marL5">대</label>
						          <input type="checkbox" id="imgMNow" name="prodBasicSizeBmsView" class="marL30"  value="M" disabled="disabled"  /><label for="imgMNow" class="marL5">중</label>
						          <input type="checkbox" id="imgSNow" name="prodBasicSizeBmsView" class="marL30"  value="S" disabled="disabled" /><label for="imgSNow" class="marL5">소</label>
		          		</div>
		          	</div>
		            <div>이미지 변경<input type="text" name="prodBasicImgName" class="verT marL5 inputPx200" style="width:200px" readonly="readonly" />
		            <a class="bisImg btn_type1  marL10" href="javascript:;">파일찾기</a>
		            <input type="checkbox" id="imgL" name="prodBasicSizeBms" class="marL30" value="B" /><label for="imgL" class="marL5">대</label>
		            <input type="checkbox" id="imgM" name="prodBasicSizeBms" class="marL30" value="M"  /><label for="imgM" class="marL5">중</label>
		            <input type="checkbox" id="imgS" name="prodBasicSizeBms" class="marL30" value="S"  /><label for="imgS" class="marL5">소</label>
		            </div>
		            ...
		            <div class="marT10">
		              ...
		              <span class="marL30">대형<input type="text" name="prodBasicSizeBmsPx" class="marLR20 alignR validation[number]" style="width:55px;"  value="607" />픽셀</span>
		              <span class="marL30">중형<input type="text"  name="prodBasicSizeBmsPx"  class="marLR20 alignR validation[number]" style="width:55px;" value="607" />픽셀</span>
		              <span class="marL30">소형<input type="text" name="prodBasicSizeBmsPx" class="marLR20 alignR validation[number]" style="width:55px;" value="105" />픽셀</span>
		            </div>
		          </td>
		        </tr>
		        <c:if test="${not empty prodDtlImg}">
		        <tr>
		          <th scope="row">현재 상세이미지</th>
		          <td colspan="3">
					<c:forEach items="${prodDtlImg}" var="prodDtlImgMpg" varStatus="status">
		      			<div class="nowDtlImg">
							...
							<div>
								  <a class="btn_type1" href="javascript:;" onclick="$.dtlImgDel('${prodDtlImgMpg.attchCd}','${status.index}');">삭제</a>
							</div>
						</div>
					</c:forEach>
					</td>
		      ...
		      <tbody>
		        ...
		        <tr class="tpP">
		         <th scope="row">소비자가격</th>
		          <td>
		           ...
		           <td>
		           	 <input type="text" id="prodPtInScore" name="prodPtInScore" class="marR5 alignR validation[number]" placeholder="0" /> P
		           </td>
		        </tr>
		        <tr class="tpP">
		        ...
		        <tr  class="tpP">
		         ...
		         <tr>
		          ...
		          <td id="inventoryQty"></td>
		          <th scope="row">추가입고</th>
		          <td>
		            <input type="text" name="prodWearingQty" class="marR5 alignR" placeholder="0" title="추가입고" /> 개
		          </td>
		        </tr>
		        <tr>
		      ...
		      <tbody  id="prodRelated">
		          ...
		          <tr>
			          <th scope="row" style="width:127px; min-width:127px;">${prodRelated.prodRelatedNm}
			          	<div class="thInfo"><a class="fancybox sme btn_type1"  data-fancybox-type="iframe" href="/mng/smartEditor.action?divName=${prodRelated.prodRelatedCd}">에디터호출</a></div>
			          	<div class="thInfo"><input type="checkbox" id="${prodRelated.prodRelatedCd}_chk" name="${prodRelated.prodRelatedCd}ContSave" value="Y" /><label for="${prodRelated.prodRelatedCd}_chk" class="marL5">기본정보로 사용</label></div>
			          </th>
			          <td>
				          ...
				          <div class="editScroll" >
				            ...
				            <input type="hidden" name="prodRelatedCd" value="${prodRelated.prodRelatedCd}" />
			          </td>
			        </tr>
		        </c:forEach>
		      </tbody>
		      ...
		      <tbody  id="prodNotif">
		          ...
		          <tr>
			          <th scope="row">${prodNotif.prodNotifNm}</th><td>
				         <input type="hidden" name="prodNotifCd" value="${prodNotif.prodNotifCd}" />
			             <input type="text"  name="prodNotifVal" style="width:96%;" value="${prodNotif.prodNotifVal}" />
			          </td>
			          <th scope="row"><input type="checkbox" id="${prodNotif.prodNotifCd}_chk" name="${prodNotif.prodNotifCd}ContSave" value="Y" /><label for="${prodNotif.prodNotifCd}_chk" class="marL5">기본정보로 사용</label>
			          </th>
			        </tr>
		        </c:forEach>
		      </tbody>
